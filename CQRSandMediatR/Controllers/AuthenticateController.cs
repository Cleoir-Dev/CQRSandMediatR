        ...
        public async Task<IActionResult> CreateUserAsync([FromBody] UserModel model)
        {
            ...
            if (userExists is not null)
                return StatusCode(
                    ...
                    new ResponseModel { Success = false, Message = "User exist!" }
                );
            ...
            if (!result.Succeeded)
            {
                var msg = result.Errors.Count() > 0 ? result.Errors.FirstOrDefault().Description : "Error create user!";
                return StatusCode(
                   StatusCodes.Status500InternalServerError,
                   new ResponseModel { Success = false, Message = msg }
               );
            }
            ...
            return Ok(new ResponseModel { Message = "User create success!" });
        }
